	...
	{ 
		...
		private bool ValidateData()
		{
			bool validate = false;
				...
				if (Name.IsEmpty())
					SourceEditorViewModel.Instance.ControllerWindow.ShowMessage("Introduzca el nombre de la carpeta");
				else if (_folderParent == null && _solution.Folders.ExistsByName(Name))
					SourceEditorViewModel.Instance.ControllerWindow.ShowMessage("Ya existe una carpeta con ese nombre en la solución");
				else if (_folderParent != null && _folderParent.Folders.ExistsByName(Name))
					SourceEditorViewModel.Instance.ControllerWindow.ShowMessage("Ya existe una carpeta con ese nombre");
				else
					validate = true;
