...
namespace Bau.Libraries.LibDocWriter.ViewModel.Helper
{
	...
	internal class ClipboardHelper
	{
		...
		///		Pega la imagen
		/// </summary>
		internal bool PasteImage(string path, int maxImageWidth, int intThumbWidth, out string fileName, out string error)
		{
			bool paste = false;
				...
				{ 
					...
					fileName = DocWriterViewModel.Instance.DialogsController.OpenDialogSave
										(Path.GetDirectoryName(fileName), "*.jpg|*.jpg", Path.GetFileName(fileName), "*.jpg");
					// Graba la imagen
					if (!string.IsNullOrEmpty(fileName))
						try
						{
							string fileClipBoard = $"{fileName}.bak";
								...
								ConvertImage(fileClipBoard, fileName, maxImageWidth, intThumbWidth);
								// Borra la imagen
